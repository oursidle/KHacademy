...
public class Test06 {
	...
	public void test() throws MessagingException, IOException {
		//목표 : 외부에 만들어 놓은 파일에 있는 HTML을 불러와 전송
		MimeMessage message = sender.createMimeMessage();
		MimeMessageHelper helper = new MimeMessageHelper(message, false, "UTF-8");
		
		helper.setTo("ourskim722@naver.com");
		...
		String text = buffer.toString();//읽어들인 결과 1차 저장
		
//		text = text.replace("#1", "바꿀 키워드");//추천하지 않음
		
		//html 해석하여 변경하는 방법(추천)
		Document doc = Jsoup.parse(text);
		Element who = doc.getElementById("who");//who라는 아이디를 가진 태그를 찾음
		who.text("고객님");//태그의 내부 text로 글자를 설정해라!
		
		Element link = doc.getElementById("link");//link라는 아이디를 가진 태그를 찾음
		link.attr("href", "https://www.google.com");//가져온 태그의 href 속성을 변경
