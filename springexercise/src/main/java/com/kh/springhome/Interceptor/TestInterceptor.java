...
/**
	인터셉터(Interceptor)
	- 완성된 스프링 프로젝트 구석구석을 간섭할 수 있도록 만들어진 도구
	
	만드는 법
	1. 상속을 받아 자격을 획득 (HandlerInterceptor)
	2. 등록 (Component)
	3. 메소드 재정의
	4. 설정파일을 만들어 어느 주소를 간섭할 것인지 설정
 */
@Component
public class TestInterceptor implements HandlerInterceptor{
	...
	/**
		...
		- false 를 반환하면 요청에 대한 작업을 차단하겠다는 의미
		- 자격 여부에 따른 접근 가능/차단 여부를 처리할 때 적합
		- request에는 사용자의 요청 정보가 들어있다(ex: 파라미터, 세션 정보, ...)
		- response에는 사용자에게 나갈 정보가 들어있다(ex: 화면정보, ...)
		- handler에는 이 요청을 처리할 대상 컨트롤러/메소드의 정보다 들어있다
	 */
	@Override
