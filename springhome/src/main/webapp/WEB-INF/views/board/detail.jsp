...
<script>
	$(function(){
		//목표: 댓글 등록을 누르면 입력적보로 ajax 통신을 통해 댓글 등록 처리
		//(주의) form은 전송이 되면 안 됨(기본 이벤트 차단, 폼은 형식으로서만 존재)
	
		$(".reply-insert-form").submit(function(e){//e: 이벤트 정보 가져옴
		...
		function loadList(){
			...
			$.ajax({
				...
				success:function(response){
					//화면 청소
					//$(".reply-list").remove();//자기 자신가지 삭제(사용 X)
					...
					for(var i=0; i < response.length; i++){
						...
						//- 지금과 같이 버튼 내부에 태그가 더 있을 때, this와 e.target은 다를 수 있음(this는 주인공, e.target은 실제 대상)
						$(htmlTemplate).find(".btn-delete").attr("data-reply-no", reply.replyNo).click(function(e){
						...
						//- 수정 시 서버로 글번호와 글 내용만 전달하면 됨
						$(htmlTemplate).find(".btn-edit").attr("data-reply-no", reply.replyNo).click(function(){
							//this == 수정버튼
...
<script id="reply-template" type="text/template">
		<div class="row flex-container view-container">
			<div class="w-75">
				...
				<div class="row left">
					<div class="replyTime">yyyy-MM-dd HH:mm:ss</div>
				</div>	
			</div>
			<div class="w-25">
				<div class="row">
