...
<h2 style="color:#F7819F">회원 관리</h2>

<c:if test="${vo.search}">
	<h3>&quot;${vo.keyword}&quot;에 대한 검색 결과</h3>
</c:if>

<table bolder="1" width="800">
	<thead>
		<tr bgcolor="#f78fb3">
			<th>아이디</th>
			<th>닉네임</th>
...
<form action="list" method="get">
	<c:choose>
		<c:when test="${param.type == 'member_id'}">
		...
		<c:when test="${param.type == 'member_contact'}">
			<select name="type" required>
				<option value="member_id">아이디</option>			
				<option value="member_nickname">닉네임</option>			
		...
		<c:when test="${param.type == 'member_birth'}">
			<select name="type" required>
				<option value="member_id">아이디</option>			
				<option value="member_nickname">닉네임</option>			
				<option value="member_contact">전화번호</option>			
