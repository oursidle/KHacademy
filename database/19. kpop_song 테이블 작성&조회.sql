...
-- 좋아요를 가장 많이 받은 곡 Top 100 (좋아요 오름차순)
select * from (
	select rownum rn, TMP.* from(
	select * from kpop_song order by kpop_song_likecount desc
	)TMP
) where rn between 1 and 100;
-- 재생을 가장 많이 한 곡 Top 100 (재생 오름차순)
select * from (
	select rownum rn, TMP.* from(
	select * from kpop_song order by kpop_song_playcount desc
	)TMP
) where rn between 1 and 100;
-- 랭킹포인트 Top 100 (랭킹포인트  오름차순)
select * from (
	select rownum rn, TMP.* from(
	select kpop_song.*, ((kpop_song_playcount*0.6)+(kpop_song_likecount*0.4)) 랭킹포인트
		from kpop_song
		order by 랭킹포인트 desc
	)TMP
) where rn between 1 and 100;
